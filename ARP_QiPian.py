...
def main():
    ...
    if len(args) != 1 or options.interface is None:
        parser.print_help()#输出参数信息
        sys.exit(0)
    mac = get_if_hwaddr(options.interface)#获取本机的mac地址
    print('本机mac地址是%s' %mac)
    
    if options.mode == 'req':#request请求
        pkt = build_req()
    elif options.mode == 'rep':#replies回复
        pkt = build_rep()

    if options.summary is True:#显示数据包发送信息
        pkt.show()
        ans = input('\n[*] 是否继续? [Y|n]: ').lower()
